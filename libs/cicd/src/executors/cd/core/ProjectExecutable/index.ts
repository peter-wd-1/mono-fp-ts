...
export const getCommitExecutorTE = matchCommand(Action.COMMIT, 'cz')
export const getReleaseExecutorTE = matchCommand(Action.RELEASE, 'release')
...
export const of = (args: ProjectExecutableArgs): ProjectExecutable => {
  const env: O.Option<number> = args.options.env === Env.PROD ? O.some(1) : O.none
  return {
